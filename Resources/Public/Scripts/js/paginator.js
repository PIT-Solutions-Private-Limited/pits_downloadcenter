    ...
    module.directive('dirPaginationControls', ['paginationService', 'paginationTemplate', function(paginationService, paginationTemplate) {
        ...
        function generatePagesArray(currentPage, collectionLength, rowsPerPage, paginationRange) {
            if(collectionLength == 0){
                ...
                document.getElementById('data-results').style.display = 'table';
            }
            document.getElementById("total-count").innerHTML=collectionLength;
            var pages = [];
        ...
        return {
            ...
            link: function dirPaginationControlsLinkFn(scope, element, attrs) {
                ...
                if (!scope.maxSize) { scope.maxSize = 4; }
                scope.directionLinks = angular.isDefined(attrs.directionLinks) ? scope.$parent.$eval(attrs.directionLinks) : true;
                scope.boundaryLinks = angular.isDefined(attrs.boundaryLinks) ? scope.$parent.$eval(attrs.boundaryLinks) : false;

                var paginationRange = Math.max(scope.maxSize, 4);
                scope.pages = [];
                scope.pagination = {
                    last: 1,
                    current: 1
